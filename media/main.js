    ...
    document.querySelectorAll('.companion-card--custom').forEach(card => {
        card.addEventListener('click', () => {
            ...
            if (fromSettings) {
                syncSettings();
                document.getElementById('companion-select-overlay').style.display = 'none';
                vscode.postMessage({ type: 'REQUEST_VRM', companion: 'custom' });
            } else {
                prefs.firstTimeDone = true;
                buildShell();
            }
...
function _finishWizard() {
    ...
    vscode.postMessage({ type: 'UPLOAD_VRM', data: _wizard.base64, name: _wizard.filename });
}
...
function buildShell() {
    ...
    <!-- Voice orb -->
    <div class="voice-wave" id="voice-wave" data-mode="idle">
      <canvas class="vw-orb" id="vw-orb"></canvas>
      <div class="wave-hint" id="wave-hint">press space to speak</div>
    </div>`;
...
function renderCompanionTab(el) {
    ...
    el.querySelector('#ss-clear-mem-confirm').addEventListener('click', () => {
        memClear();
        vscode.postMessage({ type: 'CLEAR_MEMORY' });
...
const VoiceWave = {
    ...
    setMode(mode) {
        const el = document.getElementById('voice-wave');
